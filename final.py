...
# ✅ FILTER 1: Only use matches from the last 5 years
df = df[df['match_id'] >= 113656]

# ✅ FILTER 2: Only consider overs 15 and above (finishers usually bat here)
df = df[df['over'] >= 15]

# ✅ FILTER 3: Only batters who have faced at least 100 balls overall
bat_counts = df.groupby('batter')['ball'].count()
valid_batters = bat_counts[bat_counts >= 100].index
df = df[df['batter'].isin(valid_batters)]

# ✅ Calculate clutch strike rate in overs 16-20
